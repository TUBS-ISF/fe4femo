...
def confidenceIntervals(Z, alpha=0.05, res={}):
    ...
    lower = res['stability'] - stats.norm.ppf(1 - alpha / 2) * math.sqrt(
...
multi_ind = MultiIndex.from_tuples(index, names=['feature_selector', 'ml_task'])
df = pd.DataFrame(data, index=multi_ind)
print(df)
df.to_csv(path_out)
print(df.groupby(['feature_selector']).median().sort_values(by=['stability']))
